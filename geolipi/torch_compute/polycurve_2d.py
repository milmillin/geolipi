...
def helper_polyline_bulge_arc(points, a, b, bulge):
    ...
    c2 = -end[..., 0]
    consider[2] = th.where(
        c > 0.0, th.where(c1 < 0.0 and c2 > 0.0, maxy, a[0, 1]), th.where(c1 < 0.0 or c2 > 0.0, maxy, a[0, 1])
    )

    c1 = start[..., 0]
    c2 = end[..., 0]
    consider[3] = th.where(
        c > 0.0, th.where(c1 < 0.0 and c2 > 0.0, miny, a[0, 1]), th.where(c1 < 0.0 or c2 > 0.0, miny, a[0, 1])
    )
