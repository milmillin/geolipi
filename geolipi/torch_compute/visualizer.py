...
def get_figure(sdf, input_type="SDF", res=128, point_count=2500, r=1):
    ...
    if n_points > point_count:
        # sample LIM points
        valid_points = valid_points[np.random.choice(n_points, point_count, replace=False)]
    ...
    fig = go.Figure(
        data=[
            go.Scatter3d(
                x=points[:, 0], y=points[:, 1], z=points[:, 2], mode="markers", marker=dict(size=r, color=colors)
            )
        ],
